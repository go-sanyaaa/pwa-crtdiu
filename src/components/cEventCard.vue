                ...
                v-layout(fill-height align-end).fluid.pl-2
                    v-flex.white--text
                        ...
                        v-flex(column justify-center).pt-1
                            v-chip(dark).green.ml-0.font-weight-medium.mr-3
                                v-avatar.mr-1
                                    v-icon access_time
                                | {{getHumanDate(event.event_date,"LT")}} - {{getHumanDate(event.event_date_end,"LT")}}
                            v-tooltip(bottom)
                                template(v-slot:activator="{on}")
                                    v-chip(v-on="on" outlined dark).white.font-weight-medium
                                        v-avatar.mr-1
                                            v-icon people
                                        | {{event.persons}}
            ...
            v-layout.row(justify-space-between)
                v-flex
                    ...
                    template(v-for="cat in getEventCats(event.event_cat)")
                        v-chip.mb-3.ml-0(color="blue" outlined small dark) {{cat.name}}
                v-flex.shrink.row.no-wrap
                    v-dialog(width="500" v-model="dialog")
                        template(v-slot:activator="{on}")
                            v-btn.caption(icon text v-on="on")
                        ...
                        v-card(v-if="!event.is_register")
                            ...
                            v-divider
                            v-card-actions
                                v-btn.caption(@click="dialog = false" text) Закрыть
                                v-spacer
                        ...
                        v-card(v-else)
                            ...
                            v-card-actions
                                v-btn.caption(@click="dialog = false" text) Закрыть
                                v-spacer
                                v-btn.caption(color="accent" text @click="unSubscribe") Отменить запись!
                    v-btn(icon @click="show = !show")
                        v-icon {{ show ? 'keyboard_arrow_up' : 'keyboard_arrow_down' }}
        v-fade-transition
            v-overlay( v-if="isRegister" absolute color="green" opacity="1")
        ...
        subscribe(){
            ...
            this.$store.dispatch(`events/${SUBSCRIBE_ON_EVENT}`,id)
                .then(resp => {
                    ...
                    setTimeout(()=>{this.isRegister = false},1800)
                })
                .catch(err => {
